...
// using double hashing method of open addressing
private:
    ...
    unsigned long doubleHash(string key) const {
        unsigned long hashVal = 0;
        int PRIME = 41;
        for (char letter:key) {
            hashVal = (hashVal * PRIME) + letter;
        }
        if (hashVal == 0) {
            return 1 % table.size();
        }
...
public:
    // Constructor
    OpenAddressing(unsigned long tableSize) {
    ...
    void insert(string key, Keyable value,  int &collisions) {
        if (!find(key, collisions)) {
            ...
            while (table[index].status == FILLED) {
                // send value through a second hash function
                index = index + doubleHash(key);
                // index = hash 2 (key)
                index %= table.size();
            }
            table[index].key = key;
